...
const IncomeSlide: React.FunctionComponent<IIncomeSlideProps> = ({data}) => {
  const incomeValue = {};
  ...
  data.forEach(item => {
    incomeValue[item.category.value] += item.balance;
  });

  let max = 0;
  let item: any = {};
  for (const property in incomeValue) {
    if (incomeValue[property] >= max) {
      max = incomeValue[property];
      item = {[property]: incomeValue[property]};
    }
  }
